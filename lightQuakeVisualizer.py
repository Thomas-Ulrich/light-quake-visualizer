...
def create_vtk_grid(xyz: np.ndarray, connect: np.ndarray) -> vtk.vtkUnstructuredGrid:
    ...
    cell_types = {3: vtk.VTK_POLYGON, 4: vtk.VTK_TETRA}
    grid.SetCells(cell_types[ndim2], cells)
...
parser = argparse.ArgumentParser(description="Visualize SeisSol output using pyvista")
...
parser.add_argument("--zoom", nargs=1, metavar="zoom", help="Camera zoom", type=float)
...
nfiles = len(fnames)
opacity = (
    [float(v) for v in args.opacity[0].split(";")] if args.opacity else np.ones(nfiles)
)
...
def generate_snap(mytime):
    ...
    for i, fname in enumerate(fnames):
        ...
        if args.contours:
            add_contours(plotter, grid, i, idx[0], args.contours[0])
